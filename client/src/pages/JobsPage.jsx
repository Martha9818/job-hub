  ...
  return (
    <section className="animate-fade-in">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">岗位搜索</h1>
          <p className="text-sm text-gray-500 mt-1">按城市、类别与薪资快速筛选，直达招聘页。</p>
        </div>
        {pagination.total > 0 && (
      ...
      <div className="panel p-4 mb-4">
        <div className="flex flex-wrap gap-3">
          <div className="flex flex-1 min-w-[240px]">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') updateSearch('keyword', searchInput);
              }}
            />
            <button onClick={() => updateSearch('keyword', searchInput)} className="btn-primary rounded-l-none px-4">
              搜索
            </button>
          </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 flex flex-wrap items-center gap-2">
          <span className="text-sm text-gray-500 mr-1">推荐模式：</span>
          {compactModeButtons.map(([value, label]) => (
            <button
              key={value}
              onClick={() => updateSearch('match_mode', value)}
              className={matchMode === value ? 'chip-button chip-button-active' : 'chip-button'}
            >
              {label}
            </button>
          ))}
          {matchSummary && (
            <span className="text-xs text-gray-500 ml-0 md:ml-2">
              已识别 {matchSummary.duplicates} 个重复岗位，过滤 {matchSummary.low_quality} 个低质量岗位，当前显示 {matchSummary.shown} 个
            </span>
          )}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p className="empty-state-text">暂无匹配岗位，请试试调整筛选条件</p>
        </div>
      ) : (
        <>
          <div className="space-y-3">
            {jobs.map((job) => {
              const natureTag = getNatureTag(job.experience);
              const sourceUrl = normalizeExternalUrl(job.source_url);
              return (
                <article key={job.id} className="card-interactive p-4 md:p-5 flex items-start gap-3">
                    ...
                    <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-2 text-xs md:text-sm text-gray-500">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${natureTag.className}`}>
                            {natureTag.label}
                          </span>
                          {isApplied(job.id) && (
                          ...
                          {job.match && (
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${getMatchClass(job.match.level)}`}>
                              匹配 {job.match.score}
                            </span>
                          )}
                        ...
                        {job.match?.short_reasons?.length > 0 && (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            {job.match.short_reasons.slice(0, 3).map((reason) => (
                              <span key={reason} className="text-xs px-2 py-0.5 rounded bg-gray-100 text-gray-600">
                                {reason}
                              </span>
                            ))}
                            {job.match.duplicate?.is_duplicate && (
                              <span className="text-xs px-2 py-0.5 rounded bg-yellow-100 text-yellow-700">疑似重复</span>
                            )}
                          </div>
                        )}
