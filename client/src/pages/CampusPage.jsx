import { useState, useEffect } from 'react';
import { Link, useSearchParams } from 'react-router-dom';
import { apiClient } from '../lib/api';
import { useFavorites } from '../hooks/useFavorites';
import { useAppliedJobs } from '../hooks/useAppliedJobs';
import { updateJobSearchParams } from './jobsSearchParams';

const QUICK_TABS = [
  { key: 'all', label: '全部校招', icon: '📋' },
  { key: 'spring', label: '26届春招', icon: '🌸' },
  { key: 'intern', label: '实习岗位', icon: '💼' },
  { key: 'state', label: '国企央企', icon: '🏛️' },
  { key: 'tech', label: '大厂校招', icon: '🏙️' },
  { key: 'mechanical', label: '机械专场', icon: '🔧' },
];

const HOT_CITIES = ['不限', '上海', '北京', '深圳', '杭州', '苏州', '广州', '南京', '武汉', '成都', '长沙', '合肥', '西安', '大连', '宁波'];
const EDU_OPTIONS = ['不限', '本科及以上'];

function normalizeExternalUrl(url) {
  if (!url) return '';
  if (/^https?:\/\//i.test(url)) return url;
  return `https://${url}`;
}

function getNatureTag(experience) {
  if (experience === '应届生') return { label: '校招', className: 'bg-blue-100 text-blue-700' };
  if (experience === '实习') return { label: '实习', className: 'bg-green-100 text-green-700' };
  return { label: '社招', className: 'bg-orange-100 text-orange-700' };
}

function educationText() {
  return '本科及以上';
}

function JobRow({ job, isFavorite, toggleFavorite, isApplied }) {
  const sourceUrl = normalizeExternalUrl(job.source_url);
  const nature = getNatureTag(job.experience);

  return (
    <tr className="border-b border-gray-100 hover:bg-primary-50/50 transition-colors">
      <td className="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">{job.publish_date?.slice(5) || '-'}</td>
      <td className="px-4 py-3 whitespace-nowrap">
        <span className="font-medium text-gray-800 text-sm">{job.company}</span>
      </td>
      <td className="px-4 py-3 min-w-[220px]">
        <div className="flex items-center gap-2">
          <Link to={`/jobs/${job.id}`} className="text-primary-600 hover:text-primary-800 font-medium hover:underline">
            {job.title}
          </Link>
          {isApplied(job.id) && (
            <span className="px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700">
              已投递
            </span>
          )}
        </div>
      </td>
      <td className="px-4 py-3 whitespace-nowrap">
        <span className={`px-2 py-0.5 rounded text-xs font-medium ${nature.className}`}>{nature.label}</span>
      </td>
      <td className="px-4 py-3 whitespace-nowrap">
        <span className="text-red-500 font-semibold">{job.salary_text || '面议'}</span>
      </td>
      <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{job.location || '-'}</td>
      <td className="px-4 py-3 whitespace-nowrap">
        <span className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded text-xs">{educationText()}</span>
      </td>
      <td className="px-4 py-3 whitespace-nowrap">
        <span className="px-2 py-0.5 bg-orange-50 text-orange-700 rounded text-xs">{job.category || '-'}</span>
      </td>
      <td className="px-4 py-3 text-center whitespace-nowrap">
        <div className="flex items-center justify-center gap-2">
          {sourceUrl && (
            <a href={sourceUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-primary-600 hover:underline">
              招聘页面
            </a>
          )}
          <button
            onClick={() => toggleFavorite(job.id)}
            className="text-sm hover:scale-110 transition-transform"
            title={isFavorite(job.id) ? '取消收藏' : '收藏'}
          >
            {isFavorite(job.id) ? '❤️' : '🤍'}
          </button>
        </div>
      </td>
    </tr>
  );
}

function JobCard({ job, isFavorite, toggleFavorite, isApplied }) {
  const sourceUrl = normalizeExternalUrl(job.source_url);
  const nature = getNatureTag(job.experience);

  return (
    <article className="card-interactive p-4 flex items-start gap-3">
      <Link to={`/jobs/${job.id}`} className="flex-1 min-w-0">
        <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-2">
          <div className="flex-1 min-w-0">
            <h3 className="text-base font-semibold text-gray-900 mb-1 truncate">{job.title}</h3>
            <p className="text-primary-600 font-medium text-sm mb-2">{job.company}</p>
            <div className="flex flex-wrap gap-2 text-xs text-gray-500">
              <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${nature.className}`}>
                {nature.label}
              </span>
              {isApplied(job.id) && (
                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-700">
                  已投递
                </span>
              )}
              {job.location && <span>📍 {job.location}</span>}
              <span>🎓 {educationText()}</span>
              {job.category && <span className="px-1.5 py-0.5 bg-orange-50 text-orange-600 rounded">{job.category}</span>}
            </div>
            {sourceUrl && (
              <div className="mt-2">
                <a
                  href={sourceUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-xs text-primary-600 hover:underline"
                  onClick={(event) => event.stopPropagation()}
                >
                  查看招聘页面
                </a>
              </div>
            )}
          </div>
          <div className="text-left md:text-right shrink-0">
            <div className="text-lg font-bold text-red-500">{job.salary_text || '面议'}</div>
          </div>
        </div>
      </Link>
      <button
        onClick={(event) => {
          event.preventDefault();
          toggleFavorite(job.id);
        }}
        className="shrink-0 p-1 hover:bg-gray-100 rounded-lg transition-colors"
        title={isFavorite(job.id) ? '取消收藏' : '收藏'}
      >
        {isFavorite(job.id) ? <span className="text-red-500 text-lg">❤️</span> : <span className="text-gray-300 text-lg hover:text-red-400">🤍</span>}
      </button>
    </article>
  );
}

export default function CampusPage() {
  const [searchParams, setSearchParams] = useSearchParams();
  const [jobs, setJobs] = useState([]);
  const [pagination, setPagination] = useState({ page: 1, total: 0, total_pages: 0 });
  const [loading, setLoading] = useState(true);
  const [viewMode, setViewMode] = useState('table');
  const { isFavorite, toggleFavorite } = useFavorites();
  const { isApplied } = useAppliedJobs();

  const activeTab = searchParams.get('tab') || 'all';
  const city = searchParams.get('location') || '';
  const edu = searchParams.get('education') || '';
  const keyword = searchParams.get('keyword') || '';
  const page = parseInt(searchParams.get('page') || '1', 10);

  const getTabFilters = (tab) => {
    switch (tab) {
      case 'spring':
        return { experience: '应届生', keyword: '' };
      case 'intern':
        return { experience: '实习' };
      case 'state':
        return { preset: 'state' };
      case 'tech':
        return { preset: 'tech' };
      case 'mechanical':
        return { industry: '机械' };
      default:
        return {};
    }
  };

  useEffect(() => {
    setLoading(true);
    const params = new URLSearchParams();
    const tabFilters = getTabFilters(activeTab);

    if (tabFilters.experience) params.set('experience', tabFilters.experience);
    if (tabFilters.keyword) params.set('keyword', tabFilters.keyword);
    if (tabFilters.preset) params.set('preset', tabFilters.preset);
    if (tabFilters.industry) params.set('industry', tabFilters.industry);

    const searchKeyword = keyword || '';
    if (searchKeyword && !tabFilters.keyword) params.set('keyword', searchKeyword);
    if (city && city !== '不限') params.set('location', city);
    if (edu && edu !== '不限') params.set('education', edu);
    params.set('page', String(page));
    params.set('page_size', activeTab === 'all' ? '80' : '30');

    apiClient
      .get(`/jobs?${params.toString()}`)
      .then((res) => {
        setJobs(res.data || []);
        setPagination(res.pagination || {});
      })
      .catch(() => setJobs([]))
      .finally(() => setLoading(false));
  }, [activeTab, city, edu, keyword, page]);

  const updateSearch = (key, value) => {
    setSearchParams(updateJobSearchParams(searchParams, key, value === '不限' ? '' : value));
  };

  const switchTab = (tab) => {
    const params = new URLSearchParams();
    params.set('tab', tab);
    setSearchParams(params);
  };

  const campusJobs = jobs.filter((job) => job.experience === '应届生');
  const internJobs = jobs.filter((job) => job.experience === '实习');

  const renderTable = (data) => (
    <div className="card overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              <th className="text-left px-4 py-3 font-semibold text-gray-600 whitespace-nowrap">更新</th>
              <th className="text-left px-4 py-3 font-semibold text-gray-600 whitespace-nowrap">公司</th>
              <th className="text-left px-4 py-3 font-semibold text-gray-600 whitespace-nowrap min-w-[220px]">职位</th>
              <th className="text-left px-4 py-3 font-semibold text-gray-600 whitespace-nowrap">性质</th>
              <th className="text-left px-4 py-3 font-semibold text-gray-600 whitespace-nowrap">薪资</th>
              <th className="text-left px-4 py-3 font-semibold text-gray-600 whitespace-nowrap">地点</th>
              <th className="text-left px-4 py-3 font-semibold text-gray-600 whitespace-nowrap">学历</th>
              <th className="text-left px-4 py-3 font-semibold text-gray-600 whitespace-nowrap">类别</th>
              <th className="text-center px-4 py-3 font-semibold text-gray-600 whitespace-nowrap">操作</th>
            </tr>
          </thead>
          <tbody>
            {data.map((job) => (
              <JobRow
                key={job.id}
                job={job}
                isFavorite={isFavorite}
                toggleFavorite={toggleFavorite}
                isApplied={isApplied}
              />
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );

  const renderCards = (data) => (
    <div className="space-y-3">
      {data.map((job) => (
        <JobCard
          key={job.id}
          job={job}
          isFavorite={isFavorite}
          toggleFavorite={toggleFavorite}
          isApplied={isApplied}
        />
      ))}
    </div>
  );

  return (
    <section className="animate-fade-in">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">校招实习</h1>
          <p className="text-sm text-gray-500 mt-1">按目标类型快速切换，聚焦当季可投递岗位。</p>
        </div>
        {pagination.total > 0 && (
          <span className="text-sm text-gray-600 bg-white border border-gray-200 rounded-full px-3 py-1">
            共 {pagination.total} 个岗位
          </span>
        )}
      </div>

      <div className="chip-group mb-4">
        {QUICK_TABS.map((tab) => (
          <button
            key={tab.key}
            onClick={() => switchTab(tab.key)}
            className={activeTab === tab.key ? 'chip-button chip-button-active' : 'chip-button'}
          >
            {tab.icon} {tab.label}
          </button>
        ))}
      </div>

      <div className="panel p-3 mb-4">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs text-gray-500 font-medium shrink-0">📍 城市</span>
          {HOT_CITIES.map((item) => (
            <button
              key={item}
              onClick={() => updateSearch('location', item)}
              className={(city === item || (item === '不限' && !city)) ? 'chip-button chip-button-active' : 'chip-button'}
            >
              {item}
            </button>
          ))}
        </div>
      </div>

      <div className="panel p-3 mb-6">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            <span className="text-xs text-gray-500 font-medium shrink-0">🎓 学历</span>
            {EDU_OPTIONS.map((item) => (
              <button
                key={item}
                onClick={() => updateSearch('education', item)}
                className={(edu === item || (item === '不限' && !edu)) ? 'chip-button chip-button-active' : 'chip-button'}
              >
                {item}
              </button>
            ))}
          </div>

          <div className="flex-1 min-w-[220px]">
            <input
              type="text"
              defaultValue={keyword}
              placeholder="搜索公司名、职位名..."
              className="input w-full text-sm"
              onKeyDown={(event) => {
                if (event.key === 'Enter') updateSearch('keyword', event.target.value);
              }}
            />
          </div>

          <div className="flex bg-gray-100 rounded-lg p-0.5">
            <button
              onClick={() => setViewMode('table')}
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                viewMode === 'table' ? 'bg-white shadow-sm text-primary-600' : 'text-gray-500'
              }`}
            >
              表格
            </button>
            <button
              onClick={() => setViewMode('card')}
              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                viewMode === 'card' ? 'bg-white shadow-sm text-primary-600' : 'text-gray-500'
              }`}
            >
              卡片
            </button>
          </div>
        </div>
      </div>

      {loading ? (
        <div className="text-center py-20 text-gray-400">加载中...</div>
      ) : jobs.length === 0 ? (
        <div className="empty-state">
          <div className="empty-state-icon">🔎</div>
          <p className="empty-state-text">暂无匹配的校招岗位</p>
          <p className="text-sm text-gray-400 mt-2">试试切换筛选条件</p>
        </div>
      ) : activeTab === 'all' ? (
        <div className="space-y-8">
          <div>
            <div className="flex items-center gap-3 mb-3">
              <h2 className="text-xl font-bold text-blue-700">校招专区</h2>
              <span className="text-sm text-gray-500">{campusJobs.length} 个岗位</span>
            </div>
            {campusJobs.length === 0 ? (
              <p className="text-sm text-gray-400 py-4">暂无校招岗位</p>
            ) : viewMode === 'table' ? renderTable(campusJobs) : renderCards(campusJobs)}
          </div>

          <div>
            <div className="flex items-center gap-3 mb-3">
              <h2 className="text-xl font-bold text-green-700">实习专区</h2>
              <span className="text-sm text-gray-500">{internJobs.length} 个岗位</span>
            </div>
            {internJobs.length === 0 ? (
              <p className="text-sm text-gray-400 py-4">暂无实习岗位</p>
            ) : viewMode === 'table' ? renderTable(internJobs) : renderCards(internJobs)}
          </div>
        </div>
      ) : (
        <>
          {viewMode === 'table' ? renderTable(jobs) : renderCards(jobs)}

          {pagination.total_pages > 1 && (
            <div className="flex justify-center gap-2 mt-8">
              <button
                disabled={page <= 1}
                onClick={() => updateSearch('page', String(page - 1))}
                className="btn-secondary btn-sm disabled:opacity-40"
              >
                上一页
              </button>
              <span className="px-4 py-2 text-gray-500 text-sm">
                {page} / {pagination.total_pages}
              </span>
              <button
                disabled={page >= pagination.total_pages}
                onClick={() => updateSearch('page', String(page + 1))}
                className="btn-secondary btn-sm disabled:opacity-40"
              >
                下一页
              </button>
            </div>
          )}
        </>
      )}
    </section>
  );
}
